...
st.set_page_config(layout="wide", page_title="MMM Platform")

st.title("Marketing Mix Modeling Platform (Integrated)")
st.markdown("Upload data -> Feature Engineering -> Model -> Scenarios -> Optimization")
...
if uploaded is not None:
    ...
    df = st.session_state.get('raw_data', None)

if df is None:
    st.info("Upload a CSV to start. Sample columns will be shown here when uploaded.")
    st.stop()
...
st.sidebar.subheader("Date & Target selection")
cols = df.columns.tolist()
date_col = st.sidebar.selectbox("Date column (parsed to datetime)", options=[None]+cols, index=0)
target_col = st.sidebar.selectbox("Target variable (Sales)", options=[None]+cols, index=0)

if date_col is None or target_col is None:
    st.warning("Choose a date column and target variable to proceed.")
    st.stop()
...
# Assign fiscal year
start_month = st.sidebar.selectbox("Fiscal year start month", options=list(range(1,13)), index=3, help="Common: 4 for Apr-Apr FY")
df = assign_fiscal_year(df, date_col, start_month=start_month)
st.write("Fiscal years assigned. Sample:")
st.dataframe(df[[date_col, 'Fiscal_Year']].dropna().head())

# Identify candidate media vars (numeric and not target/date)
numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
candidate_media = [c for c in numeric_cols if c not in [target_col]]
st.sidebar.subheader("Feature Engineering")
st.sidebar.write("Detected numeric columns (candidates for media):")
st.sidebar.write(candidate_media)

# Seasonal index option
with st.expander("Seasonal decomposition (SIndex)"):
    period = st.number_input("Seasonal period (weeks)", value=52, min_value=2, step=1)
    if st.button("Compute SIndex"):
        try:
            temp = df.set_index(date_col).sort_index()[target_col].fillna(method='ffill').fillna(method='bfill')
            dec = seasonal_decompose(temp, period=period, model='additive', extrapolate_trend='freq')
            df['SIndex'] = dec.seasonal.values
            st.success("SIndex added to data.")
        except Exception as e:
            st.error(f"Seasonal decomposition failed: {e}")

# Custom dummy variables
with st.expander("Custom Dummy Variables"):
    dummy_dates = st.text_input("Enter event dates (comma-separated YYYY-MM-DD)", value="")
    if st.button("Create Dummies"):
        try:
            dates = [pd.to_datetime(x.strip()) for x in dummy_dates.split(",") if x.strip()]
            for i, d in enumerate(dates, 1):
                name = f"dummy_{d.strftime('%Y_%m_%d')}"
                df[name] = (df[date_col].dt.normalize() == d.normalize()).astype(int)
            st.success(f"Created {len(dates)} dummy variables.")
        except Exception as e:
            st.error(f"Failed to create dummies: {e}")

# Variable splitting
with st.expander("Variable Split (pre/post date)"):
    split_var = st.selectbox("Select variable to split", options=[None]+candidate_media)
    split_date = st.date_input("Split date (if variable selected)", value=pd.to_datetime(df[date_col].min()))
    if split_var and st.button("Apply Split"):
        try:
            pre_mask = df[date_col] <= pd.to_datetime(split_date)
            df[f"{split_var}_pre"] = df[split_var].where(pre_mask, 0)
            df[f"{split_var}_post"] = df[split_var].where(~pre_mask, 0)
            st.success(f"Split {split_var} into pre/post at {split_date}")
        except Exception as e:
            st.error(f"Failed to split variable: {e}")

# Super campaign (combine channels)
with st.expander("Super Campaign (combine channels)"):
    to_combine = st.multiselect("Select channels to combine", options=candidate_media)
    combine_name = st.text_input("Combined name", value="combined_var")
    if to_combine and st.button("Create Super Campaign"):
        try:
            df[combine_name] = df[to_combine].sum(axis=1)
            # Optionally drop originals? checkbox
            drop_orig = st.checkbox("Drop original channels after combine", value=False)
            if drop_orig:
                df.drop(columns=to_combine, inplace=True)
            st.success(f"Created super campaign {combine_name}.")
        except Exception as e:
            st.error(f"Failed to create super campaign: {e}")

# Media spend estimation (simple CPM/CPC)
with st.expander("Media Spend Estimation"):
    spend_map = {}
    for c in candidate_media:
        if st.checkbox(f"Provide rate for {c} (CPM/CPC)?", value=False, key=f"rate_{c}"):
            rate = st.number_input(f"Rate for {c} (per 1000 for impressions or per click):", min_value=0.0, value=1.0, step=0.1, key=f"rate_input_{c}")
            spend_map[c] = rate
    if st.button("Estimate Spend"):
        for c, rate in spend_map.items():
            # assume impressions-like columns need /1000
            df[f"{c}_Spend"] = df[c] * (rate/1000.0)
        st.success("Estimated spend columns added.")

# Save engineered df to session
st.session_state['engineered_df'] = df.copy()

# --- 3) Model selection & training ---
st.sidebar.header("3. Model Selection & Training")
model_choice = st.sidebar.selectbox("Model", options=["Constrained NNLS (default)","Ridge","Lasso","ElasticNet"], index=0)
test_size = st.sidebar.slider("Test set size (fraction)", 0.05, 0.5, 0.2, 0.05)
train_button = st.sidebar.button("Train MMM model")

if train_button:
    try:
        df_train = st.session_state['engineered_df'].copy()
        # prepare base_vars and promo_vars: user picks
        st.sidebar.write("Select base variables (non-media):")
        base_vars = st.sidebar.multiselect("Base vars", options=[c for c in df_train.columns if c not in candidate_media+[target_col, date_col]])
        st.sidebar.write("Select promo variables:")
        promo_vars = st.sidebar.multiselect("Promo vars", options=candidate_media)
        # media vars - take candidate media minus base/promo selections
        media_vars = [c for c in candidate_media if c not in base_vars]
        # Run optimization of params (grid) OR use defaults
        use_opt = st.sidebar.checkbox("Auto-tune transformations (slow)", value=False)
        if use_opt:
            adp, wb = optimize_transformation_params(df_train, media_vars, target_col)
        else:
            # defaults
            adp = {m: {"decay":0.5, "lag":0} for m in media_vars}
            wb  = {m: {"k":1.5, "lam": max(1.0, df_train[m].mean() if m in df_train.columns else 1.0)} for m in media_vars}

        # Call enhanced_mmm_analysis (will apply transforms and fit constrained regression)
        artifacts = enhanced_mmm_analysis(df_train, target_col, base_vars, promo_vars, media_vars, adstock_params=adp, weibull_params=wb)
        st.session_state['model_results'] = artifacts
        st.success("Model trained and stored in session_state['model_results'].")
    except Exception as e:
        st.error(f"Training failed: {e}")

# If model results available, show diagnostics
if st.session_state.get('model_results') is not None:
    art = st.session_state['model_results']
    st.subheader("Model Diagnostics & Results")
    st.write("Features used:", art.get('feature_cols', []))
    st.write("Adstock params:", art.get('adstock_params', {}))
    st.write("Weibull params:", art.get('weibull_params', {}))
    dfm = art.get('df_model')
    if dfm is not None:
        st.write("Sample model predictions:")
        st.dataframe(dfm[[target_col, 'pred']].head())

    # Contribution table
    contrib_df = calculate_media_effectiveness(art['model'], art['df_model'], art['media_feats'])
    st.write("Media effectiveness (coef & avg contribution):")
    st.dataframe(contrib_df.round(4))

    # Plots
    fig1 = plot_avp_with_holdout(dfm, target=target_col)
    st.plotly_chart(fig1, use_container_width=True)
    fig2 = plot_contribution_by_bucket(contrib_df)
    st.plotly_chart(fig2, use_container_width=True)
    fig3 = plot_media_response_curves(st.session_state['engineered_df'], [m.replace('tr_','') for m in art['media_feats']], art['adstock_params'], art['weibull_params'])
    st.plotly_chart(fig3, use_container_width=True)

# --- 4) Scenario analysis ---
st.header("Scenario Analysis")
with st.expander("Define scenarios and compare"):
    if st.session_state.get('model_results') is None:
        st.info("Train a model first.")
    else:
        art = st.session_state['model_results']
        media_vars = [m.replace('tr_','') for m in art['media_feats']]
        sc_name = st.text_input("Scenario name", value="Increase_TV_10pct")
        selected_media = st.multiselect("Select media to change", options=media_vars)
        change_pct = st.number_input("Percent change (e.g., 10 for +10%)", value=10.0, step=1.0)
        if st.button("Run scenario"):
            changes = {m: change_pct/100.0 for m in selected_media}
            res_df = run_scenario(st.session_state['engineered_df'], art, changes, target_col)
            st.write("Scenario predicted total:", float(res_df['pred'].sum()))
            st.plotly_chart(plot_avp_with_holdout(res_df, target=target_col), use_container_width=True)

# --- 5) Optimization (Budget constraints) ---
st.header("Optimization Module")
with st.expander("Run budget-constrained optimization"):
    if st.session_state.get('model_results') is None:
        st.info("Train model first to enable optimization.")
    else:
        art = st.session_state['model_results']
        media_vars = [m.replace('tr_','') for m in art['media_feats']]
        total_budget = st.number_input("Total budget:", min_value=0.0, value=1000000.0, step=1000.0)
        st.write("Set cost per unit for each channel (for converting spend->units):")
        channel_costs = {}
        for m in media_vars:
            channel_costs[m] = st.number_input(f"Unit cost for {m}", min_value=0.0, value=1.0, key=f"cost_{m}")
        st.write("Set min/max allocation % per channel")
        minmax = {}
        for m in media_vars:
            mins = st.slider(f"Min % for {m}", 0, 100, 0, key=f"min_{m}")
            maxs = st.slider(f"Max % for {m}", 0, 100, 100, key=f"max_{m}")
            minmax[m] = (mins/100.0, maxs/100.0)
        if st.button("Run Optimization Now"):
            # Convert minmax to bounds for solver inside run_optimization if supported
            result = run_optimization(total_budget, channel_costs, art)
            if result['success']:
                st.success("Optimization succeeded")
                st.dataframe(result['optimal_allocation_df'].round(2))
                st.metric("Predicted sales (opt)", f"{result['predicted_sales']:.2f}")
            else:
                st.error("Optimization failed: " + str(result.get('message', '')))

# Footer
st.markdown("---")
st.caption("MMM Platform - interactive app.")
